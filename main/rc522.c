...
extern SemaphoreHandle_t mutexSpiBus;
static const char *TAG = "rc522";
extern Config_t config;

bool rc522TestPass = false;
/**
...
static esp_err_t rc522_spi_send(rc522_handle_t rc522, uint8_t *buffer, uint8_t length)
{
    ...
    if (config.AC1)
        gpio_set_level(RFID_CS_AC1_PIN, 0);

    err = spi_device_transmit(rc522->spi_handle, &(spi_transaction_t){
                                                     .length = 8 * length,
                                                     .tx_buffer = buffer,
                                                 });
    if (config.AC1)
...
static void rc522_task(void *arg)
{
    ...
    while (rc522->running)
    {
        ...
        if (rc522->initialized && rc522->running && rc522TestPass)
        {
            if (xSemaphoreTake(mutexSpiBus, portMAX_DELAY))
            {
